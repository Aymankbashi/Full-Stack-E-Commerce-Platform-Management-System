...
@endsection-3">
                            <span class="badge bg-white text-primary">
                                <i class="bi bi-chat-dots"></i> {{ $session->messages->count() }} رسالة
                            </span>
                            <button class="btn btn-sm btn-light" onclick="toggleTemplates()">
                                <i class="bi bi-list-check"></i> الردود الجاهزة
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="chat-input position-relative">
                        <form action="{{ route('support.agent.send', $session->id) }}" method="POST" id="message-form">
                            ...
                            <div class="input-group">
                                <button type="button" class="btn btn-light border-end-0" onclick="toggleEmoji()">
                                    <i class="bi bi-emoji-smile"></i>
                                </button>
                                <input type="text" class="form-control" name="message" id="message-input"
                        ...
                        <!-- قوالب الردود -->
                        <div class="reply-templates" id="reply-templates">
                            <h6 class="mb-3">الردود الجاهزة</h6>
...
<script>
    ...
    // إغلاق الجلسة
    function closeSession(sessionId) {
        if (confirm('هل أنت متأكد من أنك تريد إغلاق هذه الجلسة؟ سيتم حذف جميع الرسائل.')) {
            fetch(`/support/agent/close/${sessionId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    function toggleTemplates() {
        document.getElementById('reply-templates').classList.toggle('show');
        document.getElementById('emoji-picker').classList.remove('show');
    }

    // استخدام قالب رد
    function useTemplate(templateId) {
        fetch(`/support/agent/reply-templates/json`)
            .then(response => response.json())
            .then(data => {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (!e.target.closest('.emoji-picker') && !e.target.closest('button[onclick="toggleEmoji()"]')) {
            emojiPicker.classList.remove('show');
        }

        if (!e.target.closest('.reply-templates') && !e.target.closest('button[onclick="toggleTemplates()"]')) {
            replyTemplates.classList.remove('show');
        }
    });
</script>
