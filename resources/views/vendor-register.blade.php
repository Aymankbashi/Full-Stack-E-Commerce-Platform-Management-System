...
<head>
    ...
    <title>{{ $labels['vendor_register'] ?? 'إنشاء حساب تاجر' }} - MyStore</title>
...
<body>
    <div class="register-container">
        <div class="register-header">
            ...
            <h2>{{ $labels['vendor_register'] ?? 'إنشاء حساب تاجر' }}</h2>
        </div>

        <div class="vendor-info">
            <h3>{{ $labels['vendor_benefits'] ?? 'مزايات التسجيل كتاجر' }}</h3>
            <p>{{ $labels['vendor_description'] ?? 'بالتسجيل كتاجر، يمكنك:' }}</p>
            <ul>
                <li>{{ $labels['vendor_benefit1'] ?? 'عرض منتجاتك للبيع' }}</li>
                <li>{{ $labels['vendor_benefit2'] ?? 'إدارة طلبات العملاء' }}</li>
                <li>{{ $labels['vendor_benefit3'] ?? 'الوصول لوحة تحكم خاصة' }}</li>
                <li>{{ $labels['vendor_benefit4'] ?? 'تحليل مبيعاتك' }}</li>
            </ul>
        </div>
        ...
        <form method="POST" action="{{ url('/register') }}">
            ...
            <div class="form-group">
                <label for="phone">{{ $labels['phone'] ?? 'رقم الهاتف' }}</label>
                <div class="input-group">
            ...
            <div class="form-group">
                <label for="shop_name">{{ $labels['shop_name'] ?? 'اسم المتجر' }}</label>
                <div class="input-group">
            ...
            <div class="form-group">
                <label for="shop_url">{{ $labels['shop_url'] ?? 'رابط المتجر' }}</label>
                <div class="input-group">
                    <i class="fas fa-link left"></i>
                    <input type="url" id="shop_url" name="shop_url" value="{{ old('shop_url') }}" placeholder="https://example.com">
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    <select id="role" name="role" required>
                        ...
                        <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>{{ $labels['admin_role'] ?? 'مدير' }}</option>
                    </select>
                </div>
            </div>
            <div class="form-group">
                <label for="description">{{ $labels['description'] ?? 'نبذة عن متجرك' }}</label>
                <div class="input-group">
                    <i class="fas fa-align-left left"></i>
        ...
        <div class="links">
            ...
            <a href="{{ route('register') }}">{{ $labels['customer_register'] ?? 'إنشاء حساب كعميل' }}</a>
        </div>
    </div>
</body>
</html>
