...
<head>
    ...
    <title>{{ $labels['about'] ?? 'حول الشركة' }}</title>
...
<body style="direction: {{ $dir }};">
    <div class="container">
        <h2>{{ $labels['about'] ?? 'حول الشركة' }}</h2>
        <div class="section-title">{{ $labels['company_intro'] ?? 'نبذة عن الشركة' }}</div>
        ...
        <div class="section-title">{{ $labels['shipping_policy'] ?? 'سياسة الشحن' }}</div>
        <ul>
            <li>شحن سريع لجميع المناطق.</li>
            <li>توصيل مجاني للطلبات فوق 500 ريال.</li>
            <li>إمكانية تتبع الشحنة عبر الموقع.</li>
        </ul>
        <div class="section-title">{{ $labels['return_policy'] ?? 'سياسة الإرجاع' }}</div>
        <ul>
