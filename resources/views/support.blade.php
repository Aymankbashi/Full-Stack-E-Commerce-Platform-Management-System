...
<head>
    ...
    <title>{{ $labels['contact'] ?? 'دعم العملاء' }}</title>
...
<body style="direction: {{ $dir }};">
    <div class="container">
        <h2>{{ $labels['contact'] ?? 'دعم العملاء' }}</h2>
        <div class="section-title">{{ $labels['faq'] ?? 'الأسئلة الشائعة' }}</div>
        <div class="faq">
            <ul>
        ...
        <div class="section-title">{{ $labels['contact_options'] ?? 'خيارات التواصل' }}</div>
        <div class="contact-info">
            <a href="{{ route('support.landing') }}" class="chat-btn" style="display:inline-block;text-decoration:none;">{{ $labels['chat_support'] ?? 'الدعم عبر الدردشة' }}</a>
            <div style="margin-top:1rem;">{{ $labels['phone'] ?? 'رقم الهاتف' }}: <b>9200 12345</b></div>
            <div style="margin-top:0.5rem;">{{ $labels['email'] ?? 'البريد الإلكتروني' }}: <b>support@example.com</b></div>
        </div>
    </div>
</body>
</html>
